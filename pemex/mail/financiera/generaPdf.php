  ...
  if(mysql_affected_rows()>0)
  {
      ...
      while ($row = mysql_fetch_array($data)) 
      {
          if( !empty( trim($row['Texto'])  ) )
          {
            ...
            $pdf->Cell(200,4, $periodico." ".utf8_decode(  $row['Estado'])." ".$row['Fecha']." Pag. ".utf8_decode(  $row['PaginaPeriodico']) ,0,0,'L',false, $urlP ."/". $row['pdf'] );
...
class PDF extends FPDI
{
    ...
    function Header()
    {
        ...
        // Título
        $this->SetXY(150,30);
        $this->Cell(50,10, "Resumen Ejecutivo / ".utf8_decode( fecha_completa2(date('2014-10-01')) ) ,0,0,'R');
        // Salto de línea
        $this->Ln(10);
    }
...
$sql="SELECT DISTINCT(n.idEditorial), p.Nombre AS 'Periodico', n.Fecha, n.Titulo,s.seccion AS 'Seccion', n.PaginaPeriodico, n.NumeroPagina as 'PaginaPDF', REPLACE(n.Texto,'\n',' ') Texto,
  ...
  FROM (SELECT idEditorial,Fecha,Titulo , PaginaPeriodico, NumeroPagina , Texto, Periodico,Autor, Hora, Encabezado, Foto,PieFoto, Seccion, Categoria, Activo FROM  noticiasMensual WHERE Fecha='2014-10-01'
UNION ALL
SELECT idEditorial,Fecha,Titulo , PaginaPeriodico, NumeroPagina , Texto, Periodico,Autor, Hora, Encabezado, Foto,PieFoto, Seccion, Categoria, Activo FROM  noticiasAnual WHERE Fecha='2014-10-01') n, periodicos p, ordenGeneral o, estados e, seccionesPeriodicos s
  WHERE 
...
$sqlPlanas = "SELECT
        ...
        n.PieFoto
      FROM 
      (SELECT idEditorial,Fecha,Titulo , PaginaPeriodico, NumeroPagina , Texto, Autor, Periodico,Categoria,Activo FROM  noticiasMensual WHERE Fecha='2014-10-01') n, 
      periodicos p, 
      ordenGeneral o,
...
$qrySHCP = "SELECT
  ...
  n.PieFoto
FROM
  (SELECT idEditorial,Fecha,Titulo , PaginaPeriodico, NumeroPagina , Texto, Periodico,Autor, Hora, Encabezado, Foto,PieFoto, Seccion, Categoria, Activo FROM  noticiasMensual WHERE Fecha='2014-10-01'
UNION ALL
SELECT idEditorial,Fecha,Titulo , PaginaPeriodico, NumeroPagina , Texto, Periodico,Autor, Hora, Encabezado, Foto,PieFoto, Seccion, Categoria, Activo FROM  noticiasAnual WHERE Fecha='2014-10-01') n,
  periodicos p,
  ordenGeneral o,
  seccionesPeriodicos s,
  categoriasPeriodicos c,
...
$nombre = "/var/www/external/testigos/Financiera/".DATE($fechaBusca)."Reporte Financiera.pdf";
$pdf->Output($nombre, 'F'); 
