...
$queryCartones="SELECT DISTINCT(n.idEditorial), p.Nombre AS 'Periodico', n.Fecha, n.Titulo,s.seccion AS 'Seccion', n.PaginaPeriodico AS 'NumeroPagina',n.NumeroPagina as 'PaginaPDF', n.Texto,
  ...
  // Para declarar un nuevo documento
    ...
    $header->addImage('img/logo_fdn.png', $imageStyle);
    ...
    $section->addPageBreak();//PRIMERAS PLANAS
     ...
     if($resultTitular)
     {
        while ($row1 = mysql_fetch_array($resultTitular))
        {
          ...
          switch($Periodico)
          {
            case 'Financiera Rural':
              $section->addTextBreak(2); 
              $section->addText('Financiera Rural', 'estiloTitle2', 'pStyle2');
              $section->addTextBreak(1);
              $personaje=0;
            break;
            ...
            default:
                switch ($personaje)
                {
                    case 0:
                        ...
                        $section->addText(urls_amigables((textMatch($Texto,Array('Financiamiento Rural','Financiera Nacional de Desarrollo Agropecuario, Rural, Forestal y Pesquero','Financiera naciónal de Desarrollo Agropecuario','financiera rural')))),'estiloTexto','pStyle');
...
$section->addPageBreak();
    ...
    try {
        ...
        header("Access-Control-Allow-Headers: X-Requested-With");
          ...
          $objWriter->save("Sintesis_Financiera_".DATE('Y-m-d').".docx");
          chmod("Sintesis_Financiera_".DATE('Y-m-d').".docx", 0777);
...
$mail->addAddress('mariob@gacomunicacion.com', 'Mario Alberto Badillo (Hobbit)');
  ...
  $mail->FromName = utf8_decode("Financiera Rural Impresos");
