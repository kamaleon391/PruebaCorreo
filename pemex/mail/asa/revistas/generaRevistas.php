...
set_time_limit(0); 
  ...
  $fecha = date('Y-m-d');
    ...
    $query = "SELECT   n.Periodico as idPeriodico,   n.idEditorial,   n.Titulo,   p.Nombre as Periodico,   p.String_Name as StringName,   e.Nombre AS estado,   CONCAT(DATE_FORMAT(n.Fecha, '%Y-%m-%d'),' ',n.Hora) as Fecha,   n.PaginaPeriodico,   s.seccion,   c.Categoria as Categoria,   n.Autor,   n.Texto,   CONCAT('/Periodicos/', replace(  p.Nombre,' ', '%20' ),'/',n.Fecha,'/',NumeroPagina) AS 'pdf',   CONCAT('/Periodicos/', replace(  p.Nombre,' ', '%20' ),'/',n.Fecha,'/',NumeroPagina) AS 'jpg',   n.Categoria as 'Num.Categoria',   n.NumeroPagina,   n.Fecha,   n.Hora,   n.Encabezado,   n.Foto,   n.PieFoto 
FROM  
noticiasDia n,  
periodicos p,  
...
p.idPeriodico=n.Periodico AND p.idPeriodico in (463, 355, 381, 325, 367, 469, 1882, 1567, 1082, 962, 464, 1471) AND
    ...
    if($filas > 0)
    {
        while ($row = mysql_fetch_array($query1)) {
            if ($cont % 2 == 0) {
                ...
                $pdf->Image($_path_portada.$row['jpg'].'_fb.jpg', $xImg, $yImg, 80, 100, 'JPG', $_path_portada2.$row['pdf']);
                $xCell = 150;
                $xImg  = 120;
                $cont++;
            } else {
                ...
                if ($cont == 5 or $cont == 11 or $cont== 17) {
                    $pdf->AddPage();
                    $pdf->SetFont('Arial', '', 8);
