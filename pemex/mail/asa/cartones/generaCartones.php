...
p.idPeriodico=n.Periodico AND p.idPeriodico in (32, 51, 47, 50, 53, 52) AND
...
//					$pdf->Line(0,50,216,50);
					$antigua = umask(0);
					$filename = "/var/www/external/testigos/asa/cartones/".$row['Fecha']."/".$ids.'.pdf';
					if(!is_dir("/var/www/external/testigos/asa/cartones/".$row['Fecha']."/")){

						mkdir("/var/www/external/testigos/asa/cartones/".$row['Fecha'],true,0777);
						chmod("/var/www/external/testigos/asa/cartones/".$row['Fecha'],0777);
						umask($antigua);
					}
    ...
    function creaPDF($fecha, $notas){
    	...
    	$pdfs = array();
    	foreach ($notas as $x) {
    		if(!in_array("/var/www/external/testigos/asa/cartones/".$fecha."/".$x, $pdfs)){
    			$pdfs[] = "/var/www/external/testigos/asa/cartones/".$fecha."/".$x;
    		}
    	...
    	if(!empty($pdfs)){
    		...
    		mkdir("/var/www/external/testigos/asa/cartones/".$fecha."/compendio",true, 0777);
    		chmod("/var/www/external/testigos/asa/cartones/".$fecha."/compendio",0777);
    		umask(umask(0));

    		$nombre = "/var/www/external/testigos/asa/cartones/".$fecha."/compendio/".$fecha.".pdf";
    		$pdf->Output($nombre, 'F');
    	}
    }