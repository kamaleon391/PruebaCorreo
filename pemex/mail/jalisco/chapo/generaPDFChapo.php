...
set_time_limit(0); 
  ...
  $fecha = date('Y-m-d');
    ...
    n.Categoria <> 80 AND
    n.Periodico IN ( 50,51,32,53)  AND
    n.Fecha = '".$fecha."'
    AND
    (
      Titulo like '%Chapo%' OR
      Titulo like '%Joaquin Guzman%' OR
      Titulo like '%Joaquin Guzman Loera%' OR
      Titulo like '%Lider del cartel de sinaloa%' OR

      Encabezado like '%Chapo%' OR
      Encabezado like '%Joaquin Guzman%' OR
      Encabezado like '%Joaquin Guzman Loera%' OR
      Encabezado like '%Lider del cartel de sinaloa%' OR

      Texto like '%Chapo%' OR
      Texto like '%Joaquin Guzman%' OR
      Texto like '%Joaquin Guzman Loera%' OR
      Texto like '%Lider del cartel de sinaloa%'
    )
    ...
    /*
$loop_date = '2015-12-31';

while('2016-02-01' != $loop_date)
{
    $nuevafecha = strtotime ( '+1 day' , strtotime ( $loop_date ) ) ;
    $loop_date = date ( 'Y-m-d' , $nuevafecha ); 
...
FROM
    ...
    FROM
        noticiasSemana
    WHERE
        Fecha BETWEEN '2016-01-01' AND '2016-02-01' UNION SELECT 
        Fecha,
            Autor,
            Titulo,
            Encabezado,
    ...
    FROM
        noticiasAnual
    WHERE
        Fecha BETWEEN '2016-01-01' AND '2016-02-01') n,
    periodicos p,
    seccionesPeriodicos s,
    ordenGeneral o,
    categoriasPeriodicos c
WHERE
    n.periodico = o.periodico
        ...
        AND n.periodico = p.idPeriodico
        AND s.idSeccion = n.Seccion
        AND n.Categoria <> 80
        AND n.Periodico IN (50 , 51, 32, 53)
        AND n.Fecha = '".$loop_date."'
        AND (Titulo LIKE '%Chapo%'
        OR Titulo LIKE '%Joaquin Guzman%'
        OR Titulo LIKE '%Joaquin Guzman Loera%'
        OR Titulo LIKE '%Lider del cartel de sinaloa%'
        OR Encabezado LIKE '%Chapo%'
        OR Encabezado LIKE '%Joaquin Guzman%'
        OR Encabezado LIKE '%Joaquin Guzman Loera%'
        OR Encabezado LIKE '%Lider del cartel de sinaloa%'
        OR Texto LIKE '%Chapo%'
        OR Texto LIKE '%Joaquin Guzman%'
        OR Texto LIKE '%Joaquin Guzman Loera%'
        OR Texto LIKE '%Lider del cartel de sinaloa%')
GROUP BY n.Periodico , n.NumeroPagina
ORDER BY o.posicion
        ";

    creaPDF( $query , $loop_date);
}
*/
    ...
    if($filas > 0)
    {
      while( $row = mysql_fetch_array($query1) ) 
      {
        ...
        if( !empty($pdfs) ){
          ...
          $nombre="/var/www/external/testigos/Jalisco/ChapoGuzman/Reporte Chapo Guzman - ".$fecha.".pdf";
          $pdf->Output($nombre, 'F');   
