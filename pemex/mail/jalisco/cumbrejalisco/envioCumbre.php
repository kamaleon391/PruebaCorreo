...
<table width= '500px;' border='0' cellspacing='0' style='border: #CCC solid;font-family: century gothic;font-size: 15px;-moz-border-radius: 7px;-webkit-border-radius: 7px;border-radius: 7px;' align='center' >
  ...
  <tr>
    <td>&nbsp;</td>
    <td colspan='5'><a href='http://187.247.253.5/external/services/mail/jalisco/cumbrejalisco/listaTestigosCumbre.php'>REPORTE CUMBRE DE NEGOCIOS JALISCO</a></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr bgcolor='#CCCCCC'>
...
*/
  ...
  $mail->Password   = "Gacomunicacion#@2014";   
    ...
    $mail->Subject  = " Reporte Cumbre Jalisco - ".Date('Y-m-d');

    //----------------------------------------------------------------
    
    /*
    $mail->addAddress("mario.hueso@hotmail.com");
   ...
   $mail->AddBCC("rociovelag08@gmail.com");
   ...
   $mail->AddBCC("mariob@gacomunicacion.com");
    ...
    $cumbrePDFJalisco = "/var/www/external/testigos/Jalisco/CumbreJalisco/Jalisco/Reporte Cumbre Jalisco - ".DATE('Y-m-d').".pdf";
    //$cumbrePDFDF = "/var/www/external/testigos/Jalisco/CumbreJalisco/DF/Reporte Cumbre Jalisco - ".DATE('Y-m-d').".pdf";
    //$cumbrePDFEstados = "/var/www/external/testigos/Jalisco/CumbreJalisco/Estados/Reporte Cumbre Jalisco - ".DATE('Y-m-d').".pdf";


    if(file_exists( $cumbrePDFJalisco ))
    {
      $mail->AddAttachment( $cumbrePDFJalisco , "Reporte Jalisco - Cumbre Jalisco - ".DATE('Y-m-d').".pdf");
    } 
/*
    if(file_exists( $cumbrePDFDF ))
    {
      $mail->AddAttachment( $cumbrePDFDF , "Reporte DF - Cumbre Jalisco - ".DATE('Y-m-d').".pdf");
    }

    if(file_exists( $cumbrePDFEstados ))
    {
      $mail->AddAttachment( $cumbrePDFEstados , "Reporte Estados - Cumbre Jalisco - ".DATE('Y-m-d').".pdf");
    }
*/
