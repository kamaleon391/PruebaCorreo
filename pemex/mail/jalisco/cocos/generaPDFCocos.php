...
include "/var/www/external/services/mail/conexion.php";
  ...
  */
  $query = "SELECT n.Fecha, p.Nombre as 'periodico',s.seccion AS 'Seccion',c.Categoria ,n.Autor,n.Titulo,n.Encabezado, CONCAT('/var/www/Sistema-de-Captura/Periodicos/',p.Nombre,'/',n.Fecha,'/',NumeroPagina) AS 'pdf'
    ...
    n.Fecha = CURDATE()
    AND (
      Texto like '%cocos locos%' OR
      Texto like '%roberto lopez rivera%' OR
      Titulo like '%cocos locos%' OR
      Titulo like '%roberto lopez rivera%' OR
      Encabezado like '%cocos locos%' OR
      Encabezado like '%roberto lopez rivera%' OR
      Autor like '%cocos locos%' OR
      Autor like '%roberto lopez rivera%' OR
      PieFoto like '%roberto lopez rivera%' OR
      PieFoto like '%cocos locos%'
    )
    ...
    if($filas > 0)
    {
      while( $row = mysql_fetch_array($query1) ) 
      {
        ...
        if( !empty($pdfs) ){
          ...
          $nombre="/var/www/external/testigos/Jalisco/cocos/Reporte Roberto Lopez - ".DATE('Y-m-d').".pdf";
          $pdf->Output($nombre, 'F');   
