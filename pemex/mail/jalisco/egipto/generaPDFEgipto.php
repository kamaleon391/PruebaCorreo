...
$query = "SELECT n.Fecha, p.Nombre as 'periodico',s.seccion AS 'Seccion',c.Categoria ,n.Autor,n.Titulo,n.Encabezado, CONCAT('/var/www/Sistema-de-Captura/Periodicos/',p.Nombre,'/',n.Fecha,'/',NumeroPagina) AS 'pdf'
          ...
          WHERE n.Categoria <> 80 AND n.Fecha = CURDATE() AND p.Estado = 9 AND (
            Titulo like '%egipto%' OR
            Titulo like '%egipcias%' OR
            
            Encabezado like '%egipto%' OR
            Encabezado like '%egipcias%' OR

            Texto like '%egipto%'  OR
            Texto like '%egipcias%' ) AND Texto like '% mexicano%' AND Encabezado <> 'Epicentro de festejos y manifestaciones nacionales; es el referente de la ciudad'  
    ...
    if($filas > 0)
    {
      while( $row = mysql_fetch_array($query1) ) 
      {
        ...
        if( !empty($pdfs) ){
          ...
          $nombre="/var/www/external/testigos/Jalisco/Egipto/df/Reporte Egipto - DF - ".DATE('Y-m-d').".pdf";
          $pdf->Output($nombre, 'F');   
