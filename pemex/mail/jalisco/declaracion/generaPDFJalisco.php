...
$query = "SELECT n.Fecha, p.Nombre as 'periodico',s.seccion AS 'Seccion',c.Categoria ,n.Autor,n.Titulo,n.Encabezado, CONCAT('/var/www/Sistema-de-Captura/Periodicos/',p.Nombre,'/',n.Fecha,'/',NumeroPagina) AS 'pdf'
    ...
    (
      ...
      Texto like '%egipcias%' 
    ) AND Texto like '% mexicano%' AND Encabezado <> 'Epicentro de festejos y manifestaciones nacionales; es el referente de la ciudad'  
    ...
    if($filas > 0)
    {
      while( $row = mysql_fetch_array($query1) ) 
      {
        ...
        if( !empty($pdfs) ){
          ...
          $nombre="/var/www/external/testigos/Jalisco/Egipto/jal/Reporte Egipto - Jalisco - ".DATE('Y-m-d').".pdf";
          $pdf->Output($nombre, 'F');   
