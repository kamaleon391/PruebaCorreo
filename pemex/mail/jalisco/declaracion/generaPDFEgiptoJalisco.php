...
$query = "SELECT n.Fecha, p.Nombre as 'periodico',s.seccion AS 'Seccion',c.Categoria ,n.Autor,n.Titulo,n.Encabezado, CONCAT('/var/www/Sistema-de-Captura/Periodicos/',p.Nombre,'/',n.Fecha,'/',NumeroPagina) AS 'pdf'
          ...
          WHERE n.Categoria <> 80 AND n.Fecha = CURDATE() AND p.Estado = 14 AND
            (
    Texto like '%relaciones con egipto%' OR
    Texto like '%Romper relaciones%' OR
    Titulo like '%relaciones con egipto%' OR
    Titulo like '%relaciones diplomaticas con egipto%' OR
    Encabezado like '%relaciones con egipto%' OR
    Autor like '%relaciones con egipto%' OR
    PieFoto like '%relaciones con egipto%'
            ) 
    ...
    if($filas > 0)
    {
      while( $row = mysql_fetch_array($query1) ) 
      {
        ...
        if( !empty($pdfs) ){
          ...
          $nombre="/var/www/external/testigos/Jalisco/declaracion/jal/Reporte Declaraciones Egipto - Jal - ".DATE('Y-m-d').".pdf";
          $pdf->Output($nombre, 'F');   
