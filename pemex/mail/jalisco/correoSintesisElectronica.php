...
<table width= '500px;' border='0' cellspacing='0' style='border: #CCC solid;font-family: century gothic;font-size: 15px;-moz-border-radius: 7px;-webkit-border-radius: 7px;border-radius: 7px;' align='center' >
  ...
  <tr>
    <td colspan='8' align='center'><span style='font-weight: bold; font-size: 20px;'>SINTESIS DE MEDIOS JALISCO</span></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
...
/*
    ...
    */  
    ...
    $mail->FromName = "Sintesis Electronica de Medios Impresos  ".date("Y-m-d");

    $mail->Subject  = "Sintesis de Medios ".mostrar_fecha_completa(date("Y-m-d"));  
    $mail->WordWrap = 50;
    ...
    $doc_pdf = "/var/www/external/testigos/Jalisco/SintesisMediosJalisco/Sintesis Medios ".date('Y-m-d').".pdf";
    $doc_word = "/var/www/external/testigos/Jalisco/SintesisMediosJalisco/Sintesis Jalisco ".date('Y-m-d').".docx";
    

    if(file_exists( $doc_pdf ))
    {
      $mail->AddAttachment($doc_pdf);
    } 

    if(file_exists( $doc_word ))
    {
      $mail->AddAttachment($doc_word);
    } 
