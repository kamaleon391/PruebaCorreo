...
include '/var/www/external/services/mail/segob/queryCorpoGas2.php';
...
function ArmaPdf($query,$tema,$subtema, $fecha){  
  ...
  require "/var/www/external/services/mail/conexion.php";
     ...
     $antigua = umask(0);
    if(is_dir("/var/www/external/testigos/Segob/".$fecha."/")){
    }
    else{
        mkdir("/var/www/external/testigos/Segob/".$fecha,0777,true);
        chmod("/var/www/external/testigos/Segob/".$fecha,0777);
        umask($antigua);
    }   
    ...
    */
    ...
    if( mysql_affected_rows()  > 0)
    {
        ...
        $pdf->Image('/var/www/external/services/mail/segob/Logo.jpg',5,100,100); 
        ...
        for($j=0;$j<sizeof($periodico);$j++)
        {
                if(file_exists($variable[$j])){
                      ...
                      $pdf->Cell(0,0,  ($tema).  ($subtema),0,'','R');
                       $pdf->Image('/var/www/external/services/mail/segob/Logo.jpg',5,340,30); 
                     /*   $pdf->SetFont("arial", "", 9);
                      $pdf->setTextColor();
        ...
        $nombre = "/var/www/external/testigos/Segob/".$fecha."/".quita_acentos($tema).".pdf";
        $pdf->Output($nombre, 'F');      
