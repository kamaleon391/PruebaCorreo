<?php
include '/var/www/external/services/mail/sonora/querysSonora_.php';
...
require "/var/www/external/services/mail/conexion.php";
    ...
    if(mysql_affected_rows()>0)
    {
        ...
        $pdf->Rect(0, 131, 250, 40, 'F');
          ...
          $pdf->Image('/var/www/external/services/mail/sonora/logoSonora2.png',5,100,100,'PNG'); 
        ...
        for($j=0;$j<sizeof($periodico);$j++){
                if(file_exists($variable[$j])){
                      ...
                      $pdf->setTextColor();
                      $pdf->Text(170,8,$tema);
                       $pdf->Image('/var/www/external/services/mail/sonora/logoSonora2.png',5,340,30,'PNG'); 
                     /*   $pdf->SetFont("arial", "", 9);
                      $pdf->setTextColor();
        ...
        if(!is_dir("/var/www/external/testigos/Sonora/".$fecha."/")){
            mkdir("/var/www/external/testigos/Sonora/".$fecha,true,0777);
            chmod("/var/www/external/testigos/Sonora/".$fecha,0777);
            umask($antigua);
        }

        if(is_dir("/var/www/external/testigos/Sonora/".$fecha."/")){
          
          $nombre = "/var/www/external/testigos/Sonora/".$fecha."/".quita_acentos($subtema)."_".$fecha.".pdf";        
          $pdf->Output($nombre, 'F');       
        }
  ...
  require "/var/www/external/services/mail/conexion.php";
    ...
    if(mysql_affected_rows()>0)
    {
        ...
        if(is_dir("/var/www/external/testigos/Sonora/".$fecha."/")){
          ...
          $nombre = "/var/www/external/testigos/Sonora/".$fecha."/".quita_acentos($subtema)."_".date('Y-m-d').".pdf";        
          $pdf->Output($nombre, 'F');       
        }    
