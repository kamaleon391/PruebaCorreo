...
<table align="center" cellspacing="0" border="0" width=699>
 <tr style="text-align: right;">
    <td align="center"><img src="http://'.getRealIPcorreo().'/external/services/mail/inee/inee.jpg"></td>
 </tr>
...
$mensaje .= recuperaDatosWEB($qryINEE,$qryINEEWEB, $titulo = "INEE - Instituto Nacional para la Evaluación de la Educación", $buscar );
...
$mensaje .= recuperaDatosWEB($qryReformaEducativa,$qryReformaEducativaWEB, $titulo = "", $buscar );
...
$mensaje .= recuperaDatosWEB($qrySEN,$qrySENWEB, $titulo = "SEN - Sistema Educativo Nacional", $buscar );
...
$mensaje .= recuperaDatosWEB($qrySNEE,$qrySNEEWEB, $titulo = "", $buscar );
...
ORDER BY o.posicion";
$mensaje .= recuperaDatosLINKS($qryPNEELINKS, $titulo = "Politica Nacional de Evaluacion Educativa", $buscar );
...
$mensaje .= recuperaDatosWEB($qryPNEE,$qryPNEEWEB, $titulo = "Politica Nacional de Evaluacion Educativa", $buscar );
...
$mensaje .= recuperaDatosLINKS($qrySPD,$qrySPDLINKS, $titulo = "", $buscar );
...
$mensaje .= recuperaDatosWEB($qrySPD,$qrySPDWEB, $titulo = "SPD - Servicio Profesional Docente", $buscar );
...
$mensaje .= recuperaDatosLINKS($qryPRUEBALINKS, $titulo = "INEE - PRUEBA", $buscar );
...
$mensaje .= recuperaDatosWEB($qryPRUEBA,$qryPRUEBAWEB, $titulo = "INEE - PRUEBA", $buscar );
...
$mensaje .= recuperaDatosLINKS($qryEvaluacionLINKS, $titulo = "Evaluación de la Educación", $buscar );
...
$mensaje .= recuperaDatosWEB($qryEvaluacion,$qryEvaluacionWEB, $titulo = "", $buscar );
*/
...
$mensaje .= recuperaDatosLINKS($qryConferenciaLINKS, $titulo = "Conferencia del Sistema Nacional de Evaluacion Educativa", $buscar );
...
$mensaje .= recuperaDatosLINKS($qryConsejoLINKS, $titulo = "Consejo Social Consultivo de Evaluacion de la Educacion", $buscar );
...
$mensaje .= recuperaDatosLINKS($qryConsejoVinculacionLINKS, $titulo = "Consejo de Vinculacion con las Entidades Federativas", $buscar );
...
$mensaje .= recuperaDatosLINKS($qryConsejoPedagogicoLINKS, $titulo = "Consejo Pedagogico de Evaluacion Educativa", $buscar );
...
$mensaje .= recuperaDatosLINKS($qryTecnicosLINKS, $titulo = "Consejos Tecnicos Especializados", $buscar );
...
$mensaje .= recuperaDatosLINKS($qrySEPLINKS, $titulo = "SEP", $buscar );
...
ORDER BY o.posicion";
$mensaje .= recuperaDatosLINKS($qrySNTELINKS, $titulo = "SNTE", $buscar );
...
ORDER BY o.posicion";
$mensaje .= recuperaDatosLINKS($qryCNTELINKS, $titulo = "CNTE", $buscar );
...
$mensaje .= recuperaDatosWEB($qryCNTE,$qryCNTEWEB, $titulo = "", $buscar );
...
<?php
function recuperaDatosWEB($sqlPrensa,$sql, $titulo, $buscar = array())
{
    $urlP = "http://187.247.253.5";
    $resultadosPrensa=  mysql_query($sqlPrensa);
    $result =  mysql_query($sql);
    if(mysql_num_rows($resultadosPrensa)<=0 && mysql_num_rows($result)>0)
    {
        $mensaje.='
                  <tr>
...
function recuperaDatosLINKS($sql, $titulo, $buscar = array())
{
    $urlP = "http://187.247.253.5";
     //$urlP = "http://192.168.3.154";
    ...
    if(mysql_affected_rows() > 0)
    {
        ...
        while ( $row = mysql_fetch_array($result) ) 
        {
          ...
          $mensaje .='
                <tr>
                  <td colspan="3">
                    <span style="font-size:13px;"><a style="font-family: Arial Narrow;font-weight: bold;color: rgb(0, 110, 207);" href="'.$urlP.$row['pdf'].'" target="_blank">Ir al PDF</a></span> &nbsp; &nbsp;<span style="font-size:16px;"><a style="font-family: Arial Narrow;font-weight: bold;color: rgb(0, 110, 207);" href="'.$urlP.$row['jpg'].'.jpg" target="_blank">Ir a la Imagen</a></span> &nbsp;  &nbsp; Link: <span style="font-size:16px;color: blue;text-decoration: underline;"><a style="font-family: Arial Narrow;font-weight: bold;color: rgb(0, 110, 207);" href='.$urlP.$row['pdf'].' target="_blank">'.$urlP.$row['pdf'].'</a></span>
                  </td>
                </tr>'; 
        }        
