  ...
  $fecha = date('Y-m-d');
    ...
    $query = "SELECT
            ...
            WHERE 
              ...
              idPeriodico in (32,50,59,52,53,51,47,97,319,302,121,131,346,247) AND
              fecha ='".$fecha."' AND
              n.Activo=1
              GROUP BY n.NumeroPagina,p.idPeriodico
              ORDER BY  FIELD (idPeriodico,32,50,59,52,53,51,47,97,319,302,121,131,346,247)
            ";
...
function creaPDF( $query , $fecha){
  ...
  $lugar = utf8_decode('SÍNTESIS INFORMATIVA MEDIOS IMPRESOS,');
  $lugar2= utf8_decode('PORTALES, RADIO Y TELEVISIÓN');     
  ...
  $pdf->Text(10,146,strtoupper($lugar));
  $pdf->Text(10,166,strtoupper($lugar2));
  $pdf->SetFont("arial", "B", 13);
  $pdf->setTextColor(255,255,255);
  $pdf->Text(10,23,"test");

  $pdf->Image('/var/www/external/services/mail/ssa/logo.jpg',5,90,100); 

  $pdf->SetFont("arial", "B",15);
  $pdf->setTextColor();
  $pdf->Text(120,177,$fechaCompleta);   

  $portada = '/var/www/external/testigos/ssa/portada.pdf';
  //$pdf->Output($portada, 'F');
    $pdfs = array();
    ...
    if($filas > 0)
    {
      ...
      echo "Sin Registros <br>".$query."<br>";
    }   
        ...
        if( !empty($pdfs) ){
          ...
          $nombre="/var/www/external/testigos/ssa/portadas/portadas_".$fecha.".pdf";
          $pdf->Output($nombre, 'F');   

        }else{
          echo "Arreglo Vacio <br>";
          var_dump($pdfs);
          echo "<br>";
        }           
}
