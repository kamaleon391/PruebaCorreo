 ...
 */
$mensaje    = file_get_contents('/var/www/external/services/mail/tianguis/reporte.html');              // OBTENER LA PRIMERA PARTE DEL MENSAJE
$directorio = opendir("/var/www/external/testigos/tianguis/reporte/");  // RUTA ACTUAL
...
while ($archivo = readdir($directorio)) // BUCLE PARA CONSTRUIR PARTE DINAMICA DEL MENSAJE
{
    if (!is_dir($archivo) && $archivo[0]!=".") // VERIFICA NO ES DIRECTORIO
    {
      $mensaje.="<tr>
        <td>&nbsp;</td>
          <td colspan='5'><a href='http://187.247.253.5/external/testigos/tianguis/reporte/".$archivo."'><img src='http://187.247.253.5/external/services/mail/issste/icon.png' width='25' height='25'>".$archivo."</a><br><br></td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
        </tr>";
    }
...
$data   = array(
    ...
    "from" => array("gaimpresos@gacomunicacion.com", "Ga Comunicacion"),
    "subject" => "Tianguis Turistico - Resumen Final de Noticias",
    "html" => $mensaje,
