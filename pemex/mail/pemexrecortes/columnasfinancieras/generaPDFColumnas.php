...
//					$pdf->Line(0,50,216,50);
					$antigua = umask(0);
					$filename = "/var/www/external/testigos/pemex/columnasfinancieras/".$row['Fecha']."/".$ids.'.pdf';
					if(!is_dir("/var/www/external/testigos/pemex/columnasfinancieras/".$row['Fecha']."/")){

						mkdir("/var/www/external/testigos/pemex/columnasfinancieras/".$row['Fecha'],true,0777);
						chmod("/var/www/external/testigos/pemex/columnasfinancieras/".$row['Fecha'],0777);
						umask($antigua);
					}
    ...
    function creaPDF($fecha, $notas){
    	...
    	$pdfs = array();
    	foreach ($notas as $x) {
    		if(!in_array("/var/www/external/testigos/pemex/columnasfinancieras/".$fecha."/".$x, $pdfs)){
    			$pdfs[] = "/var/www/external/testigos/pemex/columnasfinancieras/".$fecha."/".$x;
    		}
    	...
    	if(!empty($pdfs)){
    		...
    		mkdir("/var/www/external/testigos/pemex/columnasfinancieras/".$fecha."/compendio",true, 0777);
    		chmod("/var/www/external/testigos/pemex/columnasfinancieras/".$fecha."/compendio",0777);
    		umask(umask(0));

    		$nombre = "/var/www/external/testigos/pemex/columnasfinancieras/".$fecha."/compendio/".$fecha.".pdf";
    		$pdf->Output($nombre, 'F');
    	}
    }
