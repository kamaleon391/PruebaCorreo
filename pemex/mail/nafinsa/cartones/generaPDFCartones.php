    ...
    function createIndividualPDF($query, $fecha){
		...
		//           $pdf->Line(30, 12, 200, 12);
				...
				//           $pdf->Line(30, 12, 200, 12);
					...
					$antigua = umask(0);
					$filename = "/var/www/external/testigos/nafinsa/cartones/".$row['Fecha']."/".$ids.'.pdf';
					if(!is_dir("/var/www/external/testigos/nafinsa/cartones/".$row['Fecha']."/")){

						mkdir("/var/www/external/testigos/nafinsa/cartones/".$row['Fecha'],true,0777);
						chmod("/var/www/external/testigos/nafinsa/cartones/".$row['Fecha'],0777);
						umask($antigua);
					}
...
ORDER BY o.posicion)";
    	...
    	/*if(!empty($pdfs)){
    		...
    		mkdir("/var/www/external/testigos/nafinsa/cartones/".$fecha."/compendio",true, 0777);
    		chmod("/var/www/external/testigos/nafinsa/cartones/".$fecha."/compendio",0777);
    		umask(umask(0));

    		$nombre = "/var/www/external/testigos/nafinsa/cartones/".$fecha."/compendio/".$fecha.".pdf";
    		$pdf->Output($nombre, 'F');
    	//}
    }
